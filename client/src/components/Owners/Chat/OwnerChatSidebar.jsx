...
import { Search, User, Circle, ChevronLeft, ChevronRight } from "lucide-react";
...
const OwnerChatSidebar = ({ onSelectRoom, selectedRoomId, userRole }) => {
  ...
  if (loading) {
    return (
      <div
        ...
        } border-r border-white/10 bg-[#31323e] flex items-center justify-center transition-all duration-300 h-full`}
      >
  ...
  return (
    <div
      className={`${
        isCollapsed ? "w-20" : "w-80"
      } border-r border-white/10 bg-[#31323e] flex flex-col transition-all duration-300 h-full relative`}
    >
      {/* Collapse Toggle Button */}
      <button
        onClick={() => setIsCollapsed(!isCollapsed)}
        className="absolute -right-3 top-6 z-10 bg-[#60519b] hover:bg-[#7d6ab8] text-white rounded-full p-1.5 shadow-lg transition-all duration-300 hover:scale-110"
          ...
          <>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-[#1e202c] border border-white/10 rounded-lg pl-10 pr-4 py-2 text-sm text-white placeholder-white/40 focus:outline-none focus:border-[#60519b]/50 transition-colors"
              />
            </div>
          </>
        )}
      ...
      {/* Chat List - Fixed height with internal scroll */}
      <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-[#60519b]/30 scrollbar-track-transparent">
        {filteredRooms.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-white/40 text-sm p-4">
            <User size={isCollapsed ? 32 : 48} className="mb-2" />
          ...
          filteredRooms.map((room) => {
            ...
            return (
              ...
              >
                {isCollapsed ? (
                  // Collapsed View - Only DP
                  <div className="relative">
                    <div className="h-12 w-12 rounded-full bg-[#60519b]/20 flex items-center justify-center overflow-hidden ring-2 ring-transparent hover:ring-[#60519b]/50 transition-all">
                    ...
                    {hasUnread && (
                      <span className="absolute -top-1 -right-1 bg-[#60519b] text-white text-xs font-bold rounded-full h-5 min-w-[20px] flex items-center justify-center px-1">
